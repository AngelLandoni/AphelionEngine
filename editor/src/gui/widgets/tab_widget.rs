use engine::egui::{Align2, Color32, CursorIcon, FontId, Frame, Pos2, Response, Rounding, Sense, Ui, Vec2};
...
pub fn render_tab_widget(ui: &mut Ui, icon: char, title: &str, is_active: bool) -> Response {
    ...
    let icon_galley = ui
        .painter()
        .layout_no_wrap(format!("{}", icon), icon_font_id, icon_color);
    ...
    if is_active {
        ...
        ui.painter().rect_filled(rect, rounding, Color32::from_hex("#242424").unwrap_or_default());
        
        /*let mut top_line_rect = rect;
        top_line_rect.max = Pos2::new(rect.max.x, rect.max.y - rect.height() + px * 2.0);
        ui.painter().rect_filled(top_line_rect, rounding, );*/
    }

    let title_pos = Pos2::new(
        (rect.left_top().x + (desired_size.x - galley.size().x) / 2.0) + icon_galley.size().x / 2.0 + SPACE_BETWEEN_ICON_AND_TITLE / 2.0,
