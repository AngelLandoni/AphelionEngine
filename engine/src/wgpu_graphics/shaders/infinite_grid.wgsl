...
fn unproject_point(x: f32, y: f32, z: f32) -> vec3<f32> {
    let unprojected = inverse4x4(camera.view_proj) * vec4<f32>(x, y, z, 1.0);
    return unprojected.xyz / unprojected.w;
}

@vertex
fn vs_main(@builtin(vertex_index) in_vertex_index: u32) -> GridOutput {
    ...
    let point: vec4<f32> = vertices[in_vertex_index];

    return GridOutput(
        vec4<f32>(point.x, point.y, 0.0, 1.0),
        unproject_point(point.x, point.y, 1.000),
        unproject_point(point.x, point.y, 0.0000001),
        vec2<f32>(point.x, point.y),
    );
}
...
@fragment
fn fs_main(in: GridOutput) -> FragOut {
