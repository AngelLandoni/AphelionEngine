...
impl Default for Projection {
    fn default() -> Self {
        Projection::Perspective { aspect_ratio: 1.0, fov: 45.0, znear: 0.1, zfar: 100.0 }
    }
}

impl Projection {
    /// Creates and returns a new `Perspective` projection using the provided
    pub fn new_perspective(aspect_ratio: f32, fov: f32, znear: f32, zfar: f32) -> Self {
        Projection::Perspective { aspect_ratio, fov, znear, zfar }
    }

    /// Creates and returns a new `Orthograpic` projection using the provided
    pub fn new_orthograpic(left: f32, right: f32, bottom: f32, top: f32, znear: f32, zfar: f32) -> Self {
        Projection::Orthograpic { left, right, bottom, top, znear, zfar }
    }
